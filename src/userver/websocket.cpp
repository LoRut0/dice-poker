...
namespace services::websocket {
    ...
    WebsocketsHandler::WebsocketsHandler(const components::ComponentConfig& config, const components::ComponentContext& context) 
        : server::websocket::WebsocketHandlerBase(config, context),
          ...
          redis_client_{context.FindComponent<components::Redis>("redisdb").GetClient("key-json")},
    ...
    int WebsocketsHandler::JsonFormatCheck(formats::json::Value &msg) const {
        std::string action;
        action = msg["action"].As<std::string>();
        if (action == "start") {
            return 2;
        }
        else if (action == "roll") {
    ...
    void WebsocketsHandler::ParseMessage(server::websocket::Message &message) const {
        ...
        switch(JsonFormatCheck(json_msg)) {
            ...
            case 0: {
                action_roll(message, json_msg);
                break;
            }
            case 1: {
                action_choose(message, json_msg);
                break;
            }
            //TODO: more_cases
        }
    ...
    {
        LOG_DEBUG() << "Started case for starting new game";

        formats::json::ValueBuilder jsonvb_for_redis;
        int64_t gameID = sqlNewOngoing();

        DefaultInit(jsonvb_for_redis, gameID);
        jsonvb_for_redis["status"] = 100;
        
        formats::json::Value json_for_redis{jsonvb_for_redis.ExtractValue()};
        message.data = formats::json::ToStableString(json_for_redis);
        redisPostJson(std::to_string(gameID), message.data);
        
        return;
    }

    void WebsocketsHandler::action_roll(server::websocket::Message &message, const formats::json::Value &json_msg) const 
    {
    ...
    void WebsocketsHandler::action_choose(server::websocket::Message &message, const formats::json::Value &json_msg) const 
    {
        ...
        if (current_state["state"]["playerq"].As<int>() == 0) current_player = "p1";
        ...
        vb["state"]["rollnum"] = 0;
        if (current_player == "p1") vb["state"]["playerq"] = 1;
        else vb["state"]["playerq"] = 0;
